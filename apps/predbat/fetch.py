...
class Fetch:
    ...
    def rate_replicate(self, rates, rate_io=None, is_import=True, is_gas=False):
        ...
        if rate_io is None:
            rate_io = {}
        # Optimise publishes a limited horizon. Historical repetition is not a
        # tariff forecast. Only the original timestamped periods are confirmed.
        source = self.get_arg("metric_octopus_import" if is_import else "metric_octopus_export", None, indirect=False)
        tariff = self.get_state_wrapper(source, attribute="tariff_code", default="") if isinstance(source, str) and not is_gas else ""
        if "NEXT_OPTIMISE" in str(tariff).upper():
            periods = self.get_state_wrapper(source, attribute="rates", default=[]) or []
            confirmed = []
            for item in periods:
                try:
                    start = datetime.fromisoformat(item["valid_from"].replace("Z", "+00:00"))
                    end = datetime.fromisoformat(item["valid_to"].replace("Z", "+00:00"))
            ...
            for minute in range(-24 * 60, self.forecast_minutes + 48 * 60):
                instant = self.midnight_utc + timedelta(minutes=minute)
                if minute not in rates or not any(start <= instant < end for start, end in confirmed):
                    rates[minute] = 26.38 if is_import else 0.0
                    quality[minute] = "unknown"
